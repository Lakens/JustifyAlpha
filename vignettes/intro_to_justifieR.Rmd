...
If we want to use the `optimal_sample` function here we need a power function that uses the total n as an input. This can often be achieved by building a small function around the intended power function. For the `optimal_alpha` example in the box before we can do a power analysis as below. 

```{r power_bla, fig.width=7}

#I ran this for two hours but it did not finish - we might need to think about a less computationally demanding example, but I think in theory this is probably how this could be done.
