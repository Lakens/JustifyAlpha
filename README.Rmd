...
author: "Daniel Lakens and Maximilian Maier"
date: "`r Sys.Date()`"
...
So far we have used only one function from the `pwr` package to specify the power function. However, any function can be entered. Below we illustrate some additional approaches. It is important to specify a power function that returns the power, where the alpha level is a variable x, and where the output is a power value between 0 and 1 (e.g., 0.8). Currently, this function only works for functions where the sample size is coded as the variable `n`. 

```{r power_alt, fig.width=7}
...
This shows that an alpha level of at least `r `round(res8,3)` would be required to avoid the Lindley paradox. INstead of the default Cauchy prior, we can also use the Unit information Bayes factor.
...
To prevent Lindley's paradox based on the unit information Bayes factor *p*-value we would need to use an alpha level of at least `r `round(res9,3)`. Of course, researchers can also use an informed prior, or use the likelihood itself (without relying on priors) by specifying the effect size of the alternative hypothesis. There are many approaches to prevent Lindley's paradox. All boil down to lowering the alpha level as a function of the sample size, but they differ slightly the relation between the sample size and the alpha level. 
